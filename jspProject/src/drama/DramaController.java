...
public class DramaController extends HttpServlet {
	...
	ServletException, IOException {
		...
		if(url.indexOf("list.do") != -1) {
			...
			List<DramaDTO> list=dao.list(start,end);
			request.setAttribute("list", list);
			request.setAttribute("page", pager);
			
			String page="/drama/list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}else if(url.indexOf("list1.do") != -1) {
			int count=dao.count();
			int curPage=1;
			if(request.getParameter("curPage") != null) {
				curPage=Integer.parseInt(request.getParameter("curPage"));
			...
			DramaDTO dto=dao.viewReplace(num);
			//BoardDTO dto=dao.view(num);
			request.setAttribute("dto", dto);
			String page="/drama/view.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			//댓글 목록 리턴
			List<DramaCommentDTO> list=dao.commentList(num);
			request.setAttribute("list", list);
			//출력 페이지 이동
			String page="/drama/comment_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}else if(url.indexOf("comment_add.do") != -1) {
			DramaCommentDTO dto =new DramaCommentDTO();
			//게시물 번호
			int drama_num=Integer.parseInt(request.getParameter("drama_num"));
			String writer=request.getParameter("writer");
			String content=request.getParameter("content");
			dto.setDrama_num(drama_num);
			dto.setWriter(writer);
			dto.setContent(content);
			dao.commentAdd(dto);
			//이곳은 백그라운드로 실행되기 떄문에 page이동이 불필요
		}else if(url.indexOf("reply.do") != -1) {
			int num=Integer.parseInt(request.getParameter("num"));
			DramaDTO dto=dao.view(num);
			dto.setContent("===게시물의 내용==\n"+dto.getContent());
			request.setAttribute("dto", dto);
			String page="/drama/reply.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			if(result != null) {//비밀번호가 맞으면
				...
				page=contextPath+"/drama_servlet/view.do?num="+num+"&message=error";
			...
			//trim() 문자열의 좌우 공백 제거
			if(filename == null || filename.trim().equals("")) {
				//새로운 첨부파일이 없을때 (테이블의 기존정보를 가져옴)
				DramaDTO dto2=dao.view(num);
			...
			//체크박에 체크되었으면, value없이 썻기 떄문에 on이 디폴트가 됨
			...
			List<DramaDTO> list=dao.searchList(search_option,keyword);
			request.setAttribute("list", list);
			request.setAttribute("search_option", search_option);
			request.setAttribute("keyword", keyword);
			String page="/drama/search.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			//BoardDTO dto=dao.view(num);
			request.setAttribute("dto", dto);
			String page="/drama/viewlog2.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			//BoardDTO dto=dao.view(num);
			request.setAttribute("dto", dto);
			String page="/drama/viewlog3.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			//BoardDTO dto=dao.view(num);
			request.setAttribute("dto", dto);
			String page="/drama/viewlog4.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			//BoardDTO dto=dao.view(num);
			request.setAttribute("dto", dto);
			String page="/drama/viewlog5.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			//BoardDTO dto=dao.view(num);
			request.setAttribute("dto", dto);
			String page="/drama/viewlog6.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			request.setAttribute("search_option", search_option);
			request.setAttribute("keyword", keyword);
			String page="/drama/searchlog.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}
	}
