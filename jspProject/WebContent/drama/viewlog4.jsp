...
$(function(){
	...
	$("#btnList").click(function(){
		location.href="${path}/drama_servlet/list1.do";
	});
	$("#btnReply").click(function(){
		document.form1.action="${path}/drama_servlet/reply.do";//답글쓰기
		document.form1.submit();
	});
	$("#btnEdit").click(function(){
		document.form1.action="${path}/drama_servlet/pass_check.do";//비밀번호체크
		document.form1.submit();
	});
});

function comment_add(){//뎃글추가
	var param="drama_num=${dto.num}&writer="+$("#writer").val()
	+"&content="+$("#content").val();
	$.ajax({
		type: "post",
		url: "${path}/drama_servlet/comment_add.do",
		data: param,
		success: function(){
			$("#writer").val("");
			$("#content").val("");
...
function comment_list(){//뎃글목록
	$.ajax({
		type: "post",
		url: "${path}/drama_servlet/commentList.do",
		data: "num=${dto.num}",
...
<%--   <tr>   
  ...
  <tr>
   <td>
     ...
     <source src="../drama/images/포레스트.mp4">
...
<%--   <tr>
  ...
  <tr>
    ...
    <td>
      ...
      <div class="container">
      <b>시청률:</b><span class="badge">6.9</span>
 <input type="button" class="btn btn-primary" value="목록" id="btnList">
  <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#myModal">스트리밍이용권</button>   
    ...
    <div class="modal-dialog modal-sm">
      ...
      <h2>드라마소개</h2>
      <h5><span class="glyphicon glyphicon-time"></span>방영일자 2020-01-29 ~ 방영중</h5>
      <p>
         심장 빼곤 다 가진 남자와 심장 빼곤 다 잃은 여자가 신비로운 숲에서 만나 자신과 숲의 비밀을 파헤쳐 가는 '강제 산골 동거 로맨스'
      </p>
</form>
